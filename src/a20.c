...
#ifdef DEBUG_XMS
#define DBG_SPRINTF(...) sprintf(__VA_ARGS__)
#else
#define DBG_SPRINTF(...)
#define logMsg(...)
#endif
...
void i15_87h(uint16_t words_to_move, uint32_t gdt_far) {
    ...
    DBG_SPRINTF(tmp, "INT15h FN 87h words_to_move: %d src: %Xh (%d) dst: %Xh (%d)",
                                words_to_move,
...
INLINE uint8_t /*BL*/ move_ext_mem_block(uint32_t tbl_addr) {
    ...
    DBG_SPRINTF(tmp, "XMS FN 0Bh LEN:%08Xh = %d (%dK)", len, len, len >> 10); logMsg(tmp);
    ...
    if (s_h == 0) { DBG_SPRINTF(tmp, "XMS FN 0Bh Src addr32:%08Xh [%04X:%04X]", s_o, s1, s0); logMsg(tmp); }
    else { DBG_SPRINTF(tmp, "XMS FN 0Bh Src addr32:%08Xh (%d)", s_o, s_h); logMsg(tmp); }
    ...
    if (d_h == 0) { DBG_SPRINTF(tmp, "XMS FN 0Bh Dst addr32:%08Xh [%04X:%04X]", d_o, d1, d0); logMsg(tmp); }
    else { DBG_SPRINTF(tmp, "XMS FN 0Bh Dst addr32:%08Xh (%d)", d_o, d_h); logMsg(tmp); }
...
INLINE uint16_t get_handle_feets(uint16_t kbs) {
    ...
    for (uint16_t i = 0; i < MAX_XMM_HANDLES; ++i) {
        ...
        DBG_SPRINTF(tmp, "get_handle_feets(%d) H:%d SZ:%d LC:%d", kbs, ph->handle, ph->sz_kb, ph->locks_cnt); logMsg(tmp);
...
bool INT_15h() {
    switch (CPU_AH) {
        ...
        case 0x24:
            switch (CPU_AL) {
                ...
                case 0x02:
                    ...
                    cf = 0;
                    CPU_AH = 0;
                    DBG_SPRINTF(tmp, "INT15! 2402 AL: 0x%X (A20 line)", CPU_AL); logMsg(tmp);
                    return true;
                case 0x03:
                    CPU_BX = 0b11;
                    CPU_AH = 0;
                    cf = 0;
                    DBG_SPRINTF(tmp, "INT15! 2403 BX: %xh", CPU_BX); logMsg(tmp);
                    return true;
            }
            break;
        case 0x87:
        ...
        case 0xE8:
            ...
            return false;
        default: {
            // DBG_SPRINTF(tmp, "INT 15h CPU_AH: 0x%X; CPU_AL: 0x%X", CPU_AH, CPU_AL); logMsg(tmp);
        }
    }
    return false;
}
    ...
    if (CPU_AH == 0x00) { // XMS 00H: Get XMS Version Number
#ifdef XMS_HMA
        DBG_SPRINTF(tmp, "XMS FN %02Xh: XMS Spec ver 3.0; Drv ver 4.0; HMA available", CPU_AH);
        CPU_DX = 0x0001; // HMA installed
#else
        DBG_SPRINTF(tmp, "XMS FN %02Xh: XMS Spec ver 3.0; Drv ver 4.0; HMA is turned off", CPU_AH);
        ...
        case 0x01: // XMS 01H: Request High Memory Area
            if (hma_in_use_count > 1) {
                DBG_SPRINTF(tmp, "XMS FN %02Xh: HMA requested to allocate %04Xh bytes (rejected - in use)", CPU_AH, CPU_DX);
                CPU_AX = XMS_ERROR_CODE; // ERROR
                CPU_BL = 0x91; // HMA is already in use
            } else if (!is_a20_line_open()) {
                DBG_SPRINTF(tmp, "XMS FN %02Xh: HMA requested to allocate %04Xh bytes (rejected - A20 is off)", CPU_AH, CPU_DX);
                CPU_AX = XMS_ERROR_CODE; // ERROR
                CPU_BL = 0x82; // A20 is OFF
            } else {
                DBG_SPRINTF(tmp, "XMS FN %02Xh: HMA requested to allocate %04Xh bytes (allocated)", CPU_AH, CPU_DX);
                hma_in_use_count++;
        ...
        case 0x02: // XMS 02H: Release High Memory Area
            hma_in_use_count = 0;
            DBG_SPRINTF(tmp, "XMS FN %02Xh: HMA requested to release", CPU_AH);
            CPU_AX = XMS_SUCCESS_CODE; // successful
            CPU_BL = 0x00;
            break;
#endif
        case 0x03: // XMS 03H: Global Enable A20
            notify_a20_line_state_changed(true);
            DBG_SPRINTF(tmp, "XMS FN %02Xh: Global Enable A20", CPU_AH);
            CPU_AX = is_a20_line_open() ? 0x0001 : 0x0000;
            CPU_BL = 0x00;
            break;
        case 0x05: // XMS 05H: Local Enable A20
            notify_a20_line_state_changed(true);
            DBG_SPRINTF(tmp, "XMS FN %02Xh: Local Enable A20", CPU_AH);
            CPU_AX = is_a20_line_open() ? 0x0001 : 0x0000;
            CPU_BL = 0x00;
            break;
        case 0x04: // XMS 04H: Global Disable A20
            notify_a20_line_state_changed(false);
            DBG_SPRINTF(tmp, "XMS FN %02Xh: Global Disable A20", CPU_AH);
            CPU_AX = is_a20_line_open() ? 0x0001 : 0x0000;
            CPU_BL = 0x00;
            break;
        case 0x06: // XMS 06H: Local Disable A20
            notify_a20_line_state_changed(false);
            DBG_SPRINTF(tmp, "XMS FN %02Xh: Local Disable A20", CPU_AH);
            CPU_AX = is_a20_line_open() ? 0x0001 : 0x0000;
            CPU_BL = 0x00;
            break;
        case 0x07: // XMS 07H: Query A20 State
            CPU_AX = is_a20_line_open() ? 0x0001 : 0x0000;
            //DBG_SPRINTF(tmp, "XMS FN 07h: Query A20 status: %s", CPU_AX ? "ON" : "OFF");
            //tmp[0] = 0;
            CPU_BL = 0x00;
            break;
        case 0x08: { // XMS 08H: Query Free Extended Memory
            ...
            DBG_SPRINTF(tmp, "XMS FN 08h: Size of largest block: %dKB of %dKB free XMS space", CPU_AX, CPU_DX);
            CPU_BL = 0;
            break;
        }
        case 0x09: { // XMS 09H: Allocate Extended Memory Block
            ...
            if (CPU_BL >= 0x80) {
                DBG_SPRINTF(tmp, "XMS FN 09h: Allocate Extended Memory Block: %dKB (rejected)", t);
                CPU_AX = XMS_ERROR_CODE;
                break;
            }
            CPU_AX = XMS_SUCCESS_CODE;
            DBG_SPRINTF(tmp, "XMS FN 09h: Allocate Extended Memory Block: %dKB (allocated #%d)", t, CPU_DX);
            break;
        }
        case 0x0A: { // XMS 0AH: Free Extended Memory Block
            ...
            DBG_SPRINTF(tmp, "XMS FN 0Ah: Free Extended Memory Block #%d; res: %02X", CPU_DX, CPU_BL);
            break;
        }
        case 0x0B:
            DBG_SPRINTF(tmp, "XMS FN 0Bh: Move Extended Memory Block: TBL %04X:%04X", CPU_DS, CPU_SI); logMsg(tmp);
            CPU_BL = move_ext_mem_block(((uint32_t)CPU_DS << 4) + CPU_SI);
            if (CPU_BL > 0x80) {
                DBG_SPRINTF(tmp, "XMS FN 0Bh: Move Extended Memory Block failed... BL: %02X", CPU_BL);
            }
            CPU_AX = CPU_BL >= 0x80 ? XMS_ERROR_CODE : XMS_SUCCESS_CODE;
            break;
        case 0x0C: {
            ...
            DBG_SPRINTF(tmp, "XMS FN 0Ch: Lock Extended Memory Block #%d %04X:%044X err: %02X", handler, CPU_DX, CPU_BX, CPU_BL);
            CPU_AX = CPU_BL >= 0x80 ? XMS_ERROR_CODE : XMS_SUCCESS_CODE;
            break;
        }
        case 0x0D:
            CPU_BL = unlock_ext_mem_block(CPU_DX);
            DBG_SPRINTF(tmp, "XMS FN 0Dh: Unlock Extended Memory Block #%d", CPU_DX);
            CPU_AX = CPU_BL >= 0x80 ? XMS_ERROR_CODE : XMS_SUCCESS_CODE;
            break;
        case 0x0E: { // XMS 0eH: Get Handle Information
            ...
            if (handle > MAX_XMM_HANDLES) {
                ...
                DBG_SPRINTF(tmp, "XMS FN 0Eh: Handle Information #%d failed (no such id)", handle);
                break;
            }
            ...
            CPU_AX = XMS_ERROR_CODE;
#endif
            DBG_SPRINTF(tmp, "XMS FN 0Eh: Handle Information #%d allocated %dKB", handle, CPU_DX);
            break;
        }
        case 0x0F: // XMS 0fH: Resize Extended Memory Block
...
#if XMS_OVER_HMA_KB
            if (CPU_DX > MAX_XMM_HANDLES) {
                DBG_SPRINTF(tmp, "XMS FN 0Fh: Resize Extended Memory Block (invalid hndl) #%d to %dKB", CPU_DX, CPU_BX);
                CPU_AX = XMS_ERROR_CODE;
                CPU_BL = 0xA2; // Invalid handler
                break;
            }
            if (CPU_BX > TOTAL_XMM_KB - 64 - (xmm_used_kb() - xmm_handle_size(CPU_DX))) {
                DBG_SPRINTF(tmp, "XMS FN 0Fh: Resize Extended Memory Block (failed) #%d to %dKB", CPU_DX, CPU_BX);
                CPU_AX = XMS_ERROR_CODE;
            ...
            CPU_AX = XMS_ERROR_CODE;
#endif
            DBG_SPRINTF(tmp, "XMS FN 0Fh: Resize Extended Memory Block #%d to %dKB", CPU_DX, CPU_BX);
            break;
#ifdef XMS_UMB
        case 0x10: // XMS 10H: Request Upper Memory Block
            ...
            DBG_SPRINTF(tmp, "XMS FN 10h: UMB allocation: BX(seg/err): %04Xh; DX(sz): %04Xh; AX(err): %04Xh", CPU_BX, CPU_DX, CPU_AX);
            break;
        case 0x11: // XMS 10H: Release Upper Memory Block
                   // DX    desired size of UMB, in paragraphs (16-byte units)
            CPU_BX = umb_deallocate(&CPU_DX, &CPU_AX);
            DBG_SPRINTF(tmp, "XMS FN 11h: UMB dellocation: BX(seg/err): %04Xh; DX(sz): %04Xh; AX(err): %04Xh", CPU_BX, CPU_DX, CPU_AX);
            break;
#endif
        ...
        default:
            DBG_SPRINTF(tmp, "XMS FN %2Xh: ERROR (not implemented)", CPU_AH);
            CPU_AX = XMS_ERROR_CODE; // ERROR
