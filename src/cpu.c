...
/*#define read86(addr32) ( \
  ((uint32_t)(addr32) < RAM_SIZE) ? RAM_read((uint32_t)addr32) : \
  ((uint32_t)(addr32) >= 0xFE000UL) ? pgm_read_byte_near((uint32_t)BIOS + (uint32_t)(addr32) - 0xFE000UL) : \
  (((uint32_t)(addr32) >= 0xB8000UL) && ((uint32_t)(addr32) < 0xBC000UL)) ? RAM_read((uint32_t)(addr32) + RAM_SIZE - 0xB8000UL) : 0 \
)*/
        ...
        switch (opcode) {
            ...
            modregrm();
                ...
                switch (reg) {
                    ...
                    case 6: op_xor8();
                        break;
                    case 7: flag_sub8(oper1b, oper2b);
                        break;
                    default:
